...
res_option = typer.Option("HD", help="Set the video resolution.", case_sensitive=False)
bpp_option = typer.Option(
    1, help="The 'bpp' or 'Bits Per Pixel' value to use.", min=1, max=3
)
...
def encode_local(
    input_path: Path = typer.Argument(
        ..., help="A path to the file you wish to encode."
    ),
...
def decode_local(
    input_path: Path = typer.Argument(
        ..., help="A path to the file you wish to decode."
    ),
    output_dir: Path = typer.Argument(
...
def test_vbr(url: str = typer.Argument(...)):
    ...
    Console().print(Downloader(url).best_vbr)
